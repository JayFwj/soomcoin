...
#if defined(NDEBUG)
# error "Soomcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc59f36d071f63c3c0d335fcda3a1ce51725fc7dc035c8c90a3b5c2b8c40beb5a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Soomcoin: starting difficulty is 1 / 2^12
...
{
    // Soomcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Soomcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Soomcoin: 3m
static const int64 nTargetSpacing = 1 * 30; // Soomcoin: 30s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Soomcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Soomcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc59f36d071f63c3c0d335fcda3a1ce51725fc7dc035c8c90a3b5c2b8c40beb5a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is soomcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x784b8c0babc9f696c39b806ea41a496d30d353c128d1378a1061f6af3e3d2433"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Soomcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SoomcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SoomcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SoomcoinMiner(CWallet *pwallet)
{
    printf("SoomcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SoomcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SoomcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SoomcoinMiner, pwallet));
}
